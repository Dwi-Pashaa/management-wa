                    ...
                    <tbody>
                        @forelse ($subscription as $item)
                            <tr>
                                ...
                                <td>{{ $item->title }}</td>
                                <td>{{ $item->limit }}</td>
                                <td>{{ $item->subscribtion_user_count }}</td>
                                <td>
                                    <a href="{{ route('subscription.show', ['slug' => $item->slug]) }}" class="btn btn-info btn-sm" title="Add User">
                                        <i class="feather icon-users"></i>
                                    </a>
                                    <a href="{{ route('subscription.edit', ['id' => $item->id]) }}" class="btn btn-warning btn-sm" title="Edit">
                                        <i class="feather icon-edit"></i>
                                    </a>
                                    <a href="javascript:void(0)" onclick="return deleteSubscription('{{ $item->id }}')" class="btn btn-danger btn-sm" title="Delete">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center">No Subscribstion</td>
                            </tr>
                        @endforelse
                    </tbody>
                    <tfoot>
